...
async function resolveUserExpense(
  userId,
  target,
  page = 1,
  pageSize = 5
  ...
  if (/^\d+$/.test(cleanTarget)) {
    ...
    if (num >= 1 && num <= pageSize) {
      const recentList = await Expense.find({
        phone: userId
      })
        .sort({ createdAt: -1 })
...
async function getRecentPageData(
  userId,
  page = 1,
  pageSize = 5
) {
  ...
  const text =
    `🕐 Recent Transactions (Page ${currentPage} of ${totalPages})\n\n` +
    `${list}\n\n` +
  ...
  transactions.forEach((t, idx) => {
    ...
    inline_keyboard.push([
      {
        text: `✏️ Edit #${displayNum}`,
        callback_data: `edit_${t._id}`
      },
      {
        text: `🗑️ Delete #${displayNum}`,
...
async function handleTelegramCommand(
  ...
  if (command === "/help") {
    return {
      text:
        ...
        "• /delete <num> - Delete a transaction (e.g. /delete 1)\n" +
  ...
  if (command === "/website") {
    ...
    const dashboardUrl =
      ...
      `phone=${encodeURIComponent(userId)}`;
  ...
  if (command === "/recent") {
    ...
    return await getRecentPageData(
      userId,
      pageNum
    );
  }
  ...
  if (command === "/budget") {
    if (args.length === 0) {
      ...
      for (const budget of budgets) {
        ...
        resultList.push(
          `• ${budget.category}: ₹${spent} / ₹${budget.limit} (${percent}%)${status}`
        );
      }
    ...
    if (args.length === 2) {
      ...
      if (isNaN(amount) || amount <= 0) {
        return (
          "⚠️ Invalid amount. Please specify a positive number.\n\n" +
          "Usage: /budget <category> <amount>\n" +
          "Example: /budget food 5000"
        );
      }
  ...
  if (command === "/delete") {
    ...
    const expense = await resolveUserExpense(
      userId,
      target
    );
    ...
    return (
      `✅ Deleted transaction: ₹${expense.amount} ` +
      `on ${expense.category}`
    );
  }
  ...
  if (command === "/edit") {
    if (args.length < 2) {
      return (
        "⚠️ Usage: /edit <number> <amount> [category]\n" +
    ...
    return (
      `✅ Updated transaction: ₹${expense.amount} ` +
      `on ${expense.category}`
    );
  }
  ...
  botInstance.on(
    ...
    async (query) => {
      ...
      if (data.startsWith("page_")) {
        ...
        const result =
          await getRecentPageData(
            userId,
            pageNum
          );

        try {
          await botInstance.answerCallbackQuery(
            query.id
          );
      ...
      // ================= DELETE =================
        ...
        if (deleted) {
          ...
          await botInstance.sendMessage(
            query.message.chat.id,
            `✅ Deleted transaction: ₹${deleted.amount} on ${deleted.category}`
          );
        } else {
          await botInstance.answerCallbackQuery(
            query.id,
...
async function handleIncomingTelegramMessage(
  ...
  if (aliasCommand) {
    try {
      ...
      console.error(
        "Telegram command processing error:",
        err
      );

      await botInstance.sendMessage(
        msg.chat.id,
        "❌ Error executing command. Try /help"
      );
    }
  } else {
    try {
      const response =
        await processFinanceMessage(
          chatId,
          text
        );
  ...
  if (
    ...
    if (telegramMode === "webhook") {
      ...
      if (webhookUrl) {
        ...
        bot.setWebHook(
          ...
          .then(() => {
            console.log(
              `Telegram webhook registered at: ${webhookUrl}`
            );
          })
          .catch(err => {
            console.error(
        ...
        console.warn(
          "TELEGRAM_MODE is webhook, but TELEGRAM_WEBHOOK_URL is not configured."
        );
      }
    ...
    // ================= POLLING MODE =================
      ...
      bot.deleteWebHook()
        .then(() =>
          console.log(
            "Cleared active Telegram webhook for polling mode."
          )
        )
        .catch(err =>
          console.error(
    ...
    console.log(
      "TELEGRAM_BOT_TOKEN not provided or default. Telegram bot inactive."
    );
  }
}
  ...
  async (req, res) => {
    ...
    if (secretToken) {
      ...
      if (
        ...
        console.warn(
          "Unauthorized Telegram webhook request: secret token mismatch"
        );
      ...
      if (update.message) {
        if (bot) {
          try {
            ...
            console.error(
              "Error handling Telegram webhook message:",
              err
            );
          }
        }
...
app.listen(PORT, () => {
  ...
  // Initialize Telegram only after
